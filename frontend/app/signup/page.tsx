...
const CSS = `
  ...
  *{box-sizing:border-box;}
  ...
  .hud-input.valid  {border-bottom-color:#00c860;box-shadow:0 0 0 1px rgba(0,160,100,0.12);}
  .hud-input.invalid{border-bottom-color:#ff4444;box-shadow:0 0 0 1px rgba(255,60,60,0.12);}
  .submit-btn-green{
    width:100%;padding:13px;border-radius:6px;
    background:linear-gradient(160deg,#0a6e44,#075c37,#043d24);
    border:1.5px solid rgba(80,200,140,0.4);color:#fff;
    font-family:'Orbitron',monospace;font-weight:700;font-size:13px;
    letter-spacing:6px;text-shadow:0 0 12px rgba(80,220,150,0.8);cursor:pointer;
    box-shadow:0 0 20px rgba(0,150,80,0.35),inset 0 1px 0 rgba(120,255,180,0.15);
    transition:filter 0.15s;
  }
  .submit-btn-green:hover{filter:brightness(1.25);}
  .link-btn{
  ...
  .led-on-green{background:#00e060;box-shadow:0 0 6px #00e060;}
...
const STR_COLORS: string[] = ["", "#ff4444", "#ff8800", "#ffcc00", "#00c860"];
const STR_LABELS: string[] = ["", "WEAK",    "FAIR",    "GOOD",    "STRONG"];

interface FormState { username: string; email: string; pass: string; }
type StatusState = "idle" | "scanning" | "complete";
...
export default function RegisterPage(): React.JSX.Element {
  ...
  const strLabel = STR_LABELS[sc] ?? "";
  const ledClass = statusSt === "scanning" ? "led led-warn" : "led led-on-green";
  ...
  return (
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, overflow: "hidden", background: "#040210", userSelect: "none" }}>
      ...
      {mounted && (
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "16px", overflowY: "auto" }}>
          ...
          <div className="panel-boot" style={{
            ...
            borderTop: "2px solid rgba(0,180,130,0.45)",
            borderRadius: "4px 4px 8px 8px",
            boxShadow: "0 0 0 1px rgba(0,60,100,0.2),0 20px 60px rgba(0,0,0,0.8)",
            ...
            <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "2px", background: "linear-gradient(90deg,transparent,rgba(0,200,130,0.7) 30%,rgba(0,160,255,0.7) 70%,transparent)" }} />

            {success ? (
              <div style={{ position: "relative", zIndex: 1, padding: "48px 28px", textAlign: "center" }}>
                <div style={{ width: "64px", height: "64px", borderRadius: "50%", border: "2px solid #00c860", margin: "0 auto 20px", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 0 30px rgba(0,200,100,0.4)", animation: "successPop 0.5s cubic-bezier(0.22,1,0.36,1) both" }}>
                  <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#00e090" strokeWidth="2.5" strokeLinecap="round">
                    <polyline points="20 6 9 17 4 12" />
                  </svg>
                </div>
                <div className="orb" style={{ fontSize: "20px", fontWeight: 900, color: "#a0ffd0", textShadow: "0 0 22px rgba(80,255,160,0.7)", letterSpacing: "0.1em", marginBottom: "10px" }}>
                  WELCOME ABOARD
                </div>
                <div className="stm" style={{ fontSize: "11px", color: "rgba(80,200,140,0.8)", letterSpacing: "2px", marginBottom: "24px" }}>
                  CREW REGISTRATION SUCCESSFUL
                </div>
                <button className="link-btn" style={{ fontSize: "12px" }} onClick={() => { window.location.href = "/login"; }}>
                  PROCEED TO AUTHENTICATION →
                </button>
              </div>
            ) : (
              <div style={{ position: "relative", zIndex: 1, padding: "24px 26px 20px" }}>
                {/* Status bar */}
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "18px", padding: "7px 12px", background: "rgba(0,0,0,0.4)", border: "1px solid rgba(0,80,140,0.3)", borderRadius: "4px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                    <div className={ledClass} style={{ animation: statusSt === "scanning" ? "statusPulse 0.6s ease-in-out infinite" : "none" }} />
                    <span className="stm" style={{ fontSize: "9px", letterSpacing: "2px", color: statusSt === "scanning" ? "#ffaa00" : statusSt === "complete" ? "#00c860" : "rgba(0,200,140,0.7)" }}>
                      {status}
                    </span>
                  </div>
                  <span className="stm" style={{ fontSize: "8px", letterSpacing: "2px", color: "rgba(0,100,150,0.5)" }}>STEP 1/1</span>
                </div>
                ...
                <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                  ...
                  <div>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "5px" }}>
                      <div className="stm" style={{ fontSize: "12px", letterSpacing: "3px", color: "rgba(0,160,255,0.5)" }}>◈ ユーザ名</div>
                      <div className="stm" style={{ fontSize: "12px", letterSpacing: "1.5px", color: "rgba(0,200,140,0.9)", border: "1px solid rgba(0,200,140,0.4)", background: "rgba(0,200,140,0.1)", padding: "2px 4px", borderRadius: "2px" }}>
                        ※あとから変更可能
                      </div>
                    </div>
                    <div style={{ position: "relative" }}>
                      <svg style={{ position: "absolute", left: "10px", top: "50%", transform: "translateY(-50%)", width: "14px", height: "14px", opacity: 0.4 }} viewBox="0 0 24 24" fill="none" stroke="#00c8ff" strokeWidth="1.5">
                        <circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/>
                      </svg>
                      <input
                        className={"hud-input" + (form.username.length > 2 ? " valid" : form.username.length > 0 ? " invalid" : "")}
                        type="text"
                        value={form.username}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setField("username", e.target.value)}
                      />
                    </div>
                  </div>
                  ...
                  {/* Email */}
                  <div>
                    <div className="stm" style={{ fontSize: "12px", letterSpacing: "3px", color: "rgba(0,160,255,0.5)", marginBottom: "5px" }}>◈ メールアドレス</div>
                    <div style={{ position: "relative" }}>
                      <svg style={{ position: "absolute", left: "12px", top: "50%", transform: "translateY(-50%)", width: "15px", height: "15px", opacity: 0.4 }} viewBox="0 0 24 24" fill="none" stroke="#00c8ff" strokeWidth="1.5">
                        <rect x="2" y="5" width="20" height="14" rx="2"/><polyline points="2 5 12 13 22 5"/>
                      </svg>
                      <input
                        className={"hud-input" + (form.email.includes("@") ? " valid" : form.email.length > 0 ? " invalid" : "")}
                        type="email"
                        value={form.email}
                  ...
                  {/* Password */}
                  <div>
                    <div className="stm" style={{ fontSize: "12px", letterSpacing: "3px", color: "rgba(0,160,255,0.5)", marginBottom: "5px" }}>◈ パスワード</div>
                    <div style={{ position: "relative" }}>
                      <svg style={{ position: "absolute", left: "12px", top: "50%", transform: "translateY(-50%)", width: "15px", height: "15px", opacity: 0.4 }} viewBox="0 0 24 24" fill="none" stroke="#00c8ff" strokeWidth="1.5">
                        <rect x="5" y="11" width="14" height="10" rx="2"/><path d="M8 11V7a4 4 0 0 1 8 0v4"/>
                      </svg>
                      <input className="hud-input" type="password"
                        value={form.pass}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => setField("pass", e.target.value)}
                      />
                    </div>
                    {form.pass.length > 0 && (
                        ...
                        <div style={{ flex: 1, display: "flex", gap: "3px" }}>
                          {[1, 2, 3, 4].map((i) => (
                            <div key={i} style={{
                              ...
                              background: i <= sc ? strColor : "rgba(0,25,55,0.7)",
                              boxShadow: i <= sc ? `0 0 4px ${strColor}` : "none",
                              transition: "all 0.3s",
                            }} />
                          ))}
                        </div>
                        <span className="stm" style={{ fontSize: "8px", letterSpacing: "2px", color: strColor, minWidth: "46px" }}>{strLabel}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <button type="submit" className="submit-btn-green orb" disabled={statusSt === "scanning"}>
                    {statusSt === "scanning" ? "REGISTERING..." : "登録"}
                  </button>
                </form>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "12px", margin: "16px 0 12px" }}>
                  ...
                  <span className="stm" style={{ fontSize: "8px", letterSpacing: "3px", color: "rgba(0,100,150,0.5)" }}>EXISTING CREW</span>
                  <div style={{ flex: 1, height: "1px", background: "rgba(0,80,140,0.3)" }} />
                </div>
                <div style={{ textAlign: "center" }}>
                  <button className="link-btn" style={{ fontSize: "16px" }} onClick={() => { window.location.href = "/login"; }}>ログイン</button>
                </div>
              </div>
            )}
            ...
            <div style={{ borderTop: "1px solid rgba(0,60,120,0.3)", padding: "8px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "relative", zIndex: 1 }}>
              <span className="stm" style={{ fontSize: "8px", letterSpacing: "2px", color: "rgba(0,80,120,0.5)" }}>SEC-LVL: BRAVO</span>
              <div style={{ display: "flex", gap: "8px" }}>
                {(["led-on-green","led-on-green","led-on-green","led-dim"] as string[]).map((s, i) => (
          ...
          <div className="panel-boot stm" style={{ marginTop: "14px", fontSize: "8px", letterSpacing: "3px", color: "rgba(0,80,130,0.5)" }}>
